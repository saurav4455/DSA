...
int main()
{
    ...
    FILE *inputFile;
    if ((inputFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\inputFile.txt","w")) == NULL)
     {
    ...
    FILE *stackFile;
    int num;
    inputFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\inputFile.txt","r");
    stackFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\stackFile.txt","w");
    ...
    for(;;)
    {
        ...
        switch(choice)
        {
            case 1: //push to pushFile
                    num=peak(&s);
                    pushFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\pushFile.txt","a");
                    fprintf(pushFile,"%d\n",num);
                    fclose(pushFile);
                    pop(&s);
                    logFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\logFile.txt","a");
            ...
            case 2: //to put into popFile
                    num=peak(&s);
                    popFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\popFile.txt","a");
                    fprintf(popFile,"%d\n",num);
                    fclose(popFile);
                    pop(&s);
                    logFile = fopen("D:\\COLLEGE\\sem-4\\dsa\\DATA STRUCTURE\\push_pop_log\\logFile.txt","a");
